/*
 * $Id: maxent.cpp,v 1.5 2011-05-06 13:01:06 matuzaki Exp $
 */
...
static ME_Model * pointer_to_working_object = NULL; // this is not a good solution...
...
double
ME_Model::FunctionGradientWrapper(const vector<double> & x, vector<double> & grad)
{
...
int
ME_Model::perform_BFGS()
{
  cerr << "performing BFGS" << endl;
  ...
  if (_inequality_width > 0) {
    x = perform_OWLQN(ME_Model::FunctionGradientWrapper, x0, _inequality_width);
  } else {
    x = perform_LBFGS(ME_Model::FunctionGradientWrapper, x0);
  }
                                  ...
                                  std::vector<double> & membp) const
{
  ...
  const double offset = max(0.0, *pmax - 700); // to avoid overflow
  for (int label = 0; label < _num_classes; label++) {
    const double pow = powv[label] - offset;
    double prod = exp(pow);
    //      cout << pow << " " << prod << ", ";
...
ME_Model::get_features(list< pair< pair<string, string>, double> > & fl)
{
  ...
  for (StrDic::const_Iterator i = _featurename_bag.begin();
    ...
    for (int j = 0; j < _label_bag.Size(); j++) {
      ...
      const int id = _fb.Id(ME_Feature(j, i.getId()));
      if (id < 0) continue;
 ...
 *
 * Revision 1.3  2007/06/26 08:41:42  tsuruoka
 * bugfix: conditional_probability()
 *
